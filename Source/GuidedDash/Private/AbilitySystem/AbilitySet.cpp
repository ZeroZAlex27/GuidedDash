...
void FAbilitySet_GrantedHandles::TakeFromAbilitySystem(UGuidedAbilitySystemComponent* ASC)
{
	check(ASC);
...
void UAbilitySet::GiveToAbilitySystem(UGuidedAbilitySystemComponent* ASC, FAbilitySet_GrantedHandles* OutGrantedHandles, UObject* SourceObject) const
{
	check(ASC);
	...
	for (int32 AbilityIndex = 0; AbilityIndex < GrantedGameplayAbilities.Num(); ++AbilityIndex)
	{
		...
		UGuidedGameplayAbility* AbilityCDO = AbilityToGrant.Ability->GetDefaultObject<UGuidedGameplayAbility>();
